...
module YKProto
  ...
  class Client
    ...
    PacketHeader = Struct.new(:state, :type, :checksum, keyword_init: true)
      ...
      loop do
        status = with_timeout(timeout) do
          if rand > lost_packets_ratio
            socket.send([state, *YKProto::Checksum.compute(data), *data].pack("C*"), 0, to[:host], to[:port])
            puts "sent #{data}"
          end
    ...
    def receive
      Thread.new do
        ...
        Socket.udp_server_loop(to[:host], to[:port]) do |message, sender|
          if message.bytes.size > 5
            checksumMatched, computed = YKProto::Checksum.check(message.bytes[5..], message.bytes[1..4])
            unless checksumMatched
              puts "[warn]: unexpected checksum, received #{message.bytes[1..4]}, computed: #{computed}"
            end
          end

          if message.bytes[0] == state
            print message.bytes[5..].pack("C*")
            @state = 0x1 - state
          end
          ...
          if data.size > 0 && rand > lost_packets_ratio
            sender.reply([YKProto::MSG_FLAG, 0b0, *data[0].bytes].pack("C*"))
            @data = data[1..]
          end
